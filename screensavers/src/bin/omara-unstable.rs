// omara-unstable - Chaotic explode + dramatic snap-back screensaver
...
use omara_screensavers::{branding, effects::unstable::create_unstable_effect};
...
fn run<B: ratatui::backend::Backend>(terminal: &mut Terminal<B>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut effect = create_unstable_effect(&art);
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let p = Paragraph::new(text).style(Style::default().fg(Color::Rgb(255, 140, 80)));
